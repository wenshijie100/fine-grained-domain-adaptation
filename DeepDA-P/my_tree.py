# -*- coding: utf-8 -*-
"""
Created on Wed Oct 27 11:27:18 2021
...
class Myclass(object):
    def __init__(self, pos,value,vn,cnt,snt):
        self.pos = pos
        self.value = np.array(value)
        self.vn=vn
        self.cnt = cnt
        self.snt=snt #cnt before delete
...
def build_tree(vn,cnt,K):#特征维度，总节点数，K叉树
    ...
    tree=[ Myclass(0,t,vn,0,0)for i in range(pow(K,log_cnt))]
...
def update_tree(num,level,left,right,K,L):
    ...
    while(True):
        ...
        if mx<0.2*tree[num].vn*(1+float(len(L)-level)/10):
           break
        ...
        tree[num].value=tree[num].value/tree[num].cnt
    if tree[num].cnt<0.3/(1+float(len(L)-level)/10)*tree[num].snt:
        tree[num].cnt=0
    '''
    F,S=init_link(K)
    for i in range(K):
...
def K_tree(X_train_o,Y_train,pre_label,h=1,K=6):  

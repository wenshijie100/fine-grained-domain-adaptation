# -*- coding: utf-8 -*-
"""
Created on Thu Oct 21 21:36:02 2021
...
import my_loadtest_77G as myloadtest77G
import my_normlization as mynormlization
import my_loadtest_own as myloadtestown
...
class Myclass(object):
    def __init__(self, pos,value,vn,cnt):
        self.pos = pos
    ...
    def clean(self):
        self.color = 0
        self.value = np.array([0 for i in range(self.vn)])
        self.cnt = 0
tree=[]
st=[]
...
def update_tree(num,level,left,right,K,L):
    ...
    if S[root_num]<0.35*tree[num].cnt:
...
X_train_o,Y_train=myloadtestown.load_dataset_test(h=289)
X_train,CC2=get_tsfresh3(X_train_o)
X_train=np.array(X_train)
print("GGG:",X_train.shape,Y_train.shape)     
K=6   

...
class TransferLoss(nn.Module):
    def __init__(self, loss_type, **kwargs):
        ...
        if loss_type == "mmd":
            ...
            self.loss_func = PMMDLoss(**kwargs)
        elif loss_type == "twommd":
            self.loss_func = TWOMMDLoss(**kwargs)
        elif loss_type == "threemmd":
            self.loss_func = THREEMMDLoss(**kwargs)
        elif loss_type == "dwl":
            self.loss_func = DynamicWeightLoss(**kwargs)
        elif loss_type == "only":

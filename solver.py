...
class Predictor(nn.Module):
    def __init__(self, in_feature,prob=0.5):
...
class Solver(nn.Module):
    ...
    '''
    def __init__(self,n_class ,myPI,max_iter, source, target, learning_rate, batch_size,method, **kwargs):
        ...
        self.num_k3 = 8
        self.num_k4 = 1
        self.G=resnet_orginal.resnet34()
        self.C=Predictor(512)
        self.C1=Predictor(512)
        self.C2=Predictor(512)
        self.D=AdversarialNetwork(512)
        ...
        #print("BATCH_SZIE:",self.batch_size)
        self.method=method
        if self.method=='True':
    ...
    def train(self, A_st_min,A_st_max, min_J_w, max_J_w, A_st_norm, J_w_norm, epoch,source_loader, target_train_loader,args):
...
################ the balance of transferability and discriminability ###########
            for i3 in range(self.num_k3):
                ...
                if args.method=="True":
                    loss_p=self.person_loss(feat_cr_s,feat_cr_t)
                    loss_5 = loss_5+args.myweight*loss_p
...
#================ data for A_st and J_w ==============================#
	...
	#between_class scatter matrix
        ...
        if n_dim > len(clusters)-1:
            print("K is too much target")
            print("please input again")
            for j in range(4):
                if j not in np.unique(label_for_LDA):
                    label_for_LDA[j]=j
            clusters = np.unique(label_for_LDA)
...
#test--------------test---------------------------
    def test(self, target_test_loader):
        self.G.eval()
        self.C.eval()
        self.C1.eval()
        self.C2.eval()
        ...
        for data, target,weight,person,item  in target_test_loader:
            ...
            size += k
            pred_t,label_t=pred1.cpu(),label.data.cpu()
            for i in range(len(label_t)):
                preds.append(pred_t[i])
                labels.append(label_t[i])

...
class DynamicWeightLoss(nn.Module):
    ...
    def forward(self, source, target,source_label,A_st_min,A_st_max, min_J_w, max_J_w, A_st_norm, J_w_norm):
        lamb = 1.0
        img_s=source
        img_t=target   
        label_s=source_label     
...
##########################compute the T #############
        for i1 in range(self.num_k1):
            ...
            #print("SS:",feat_cr_s.size())
            output_cr_s_C = self.C(feat_cr_s.cuda())
            #print("OUTPUT:",output_cr_s_C.size())
            loss_cr_s = criterion(output_cr_s_C, source_label) 
            #print("LOSS:",loss_cr_s)
            loss_1 = loss_cr_s 
            print("L1:", loss_1)
            loss_1.backward()
            self.opt_c.step()
...
###################### discriminablity ##################
            ...
            loss_cr_s = criterion(output_cr_s_C1, source_label) + criterion(output_cr_s_C2, source_label) + criterion(output_cr_s_C, source_label)
...
############ re-weiting based on the uncertainity of pseudo-label ###############
        for i4 in range(self.num_k4):
            ...
            for ii in range(output_cr_t_C_de.size()[0]):
